...
export default function ({ feature }: { feature: Feature }) {
  ...
  const upviteDownvote = (upvote: boolean) => {
    if (
  ...
  return (
    <div className="flex flex-col items-center">
      <button onClick={() => upviteDownvote(true)} className={feature.user_has_upvoted ? "text-amber-600" : ""}>
        <svg
      ...
      <button onClick={() => upviteDownvote(false)} className={feature.user_has_downvoted ? "text-amber-600" : ""}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
